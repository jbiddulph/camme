...
if (btnStartBroadcast) {
  btnStartBroadcast.addEventListener('click', async () => {
    ...
    const response = await fetch(`${API_BASE}/broadcast/start`, {
      method: 'POST',
      headers: {
        ...authHeaders(),
      },
...
async function loadLiveBroadcasters() {
  ...
  liveBroadcasters.innerHTML = items
    .map((item) => {
      ...
      return `<div style="margin-bottom:12px;">${img}<p><strong>${item.display_name}</strong></p><p><a href="${watch}">Watch live</a></p></div>`;
    })
    .join('');
}
